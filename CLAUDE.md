...
- `renderer/`: Abstract Renderer API, Shaders, Textures, Buffers, Framebuffers (RGBA16F for HDR scene, DEPTH24STENCIL8 + DEPTH32F for shadow maps), VertexArray, Cameras (`OrthographicCamera`, `EditorCamera`), `Renderer2D` (writes linearized values into the HDR framebuffer — `quad.frag` / `circle.frag` / `line.frag` / `grid.frag` apply `pow(rgb, 2.2)` at output), `Renderer3D` (mesh facade — PBR via Cook-Torrance direct lighting + Karis split-sum IBL, raw linear HDR output, 4-cascade CSM shadow sampling, scene-owned environment with editor fallback; owns the unified post pipeline **Bloom → ACES Tonemap → FXAA**, `DrawSkybox`, and a one-time 512² RG16F BRDF LUT generated at `Init`), `TextureCubemap` (`CreateFromEquirect` / `CreateIrradiance` / `CreatePrefiltered`), `MeshAsset` (GLTF/GLB import via cgltf, with embedded-texture extraction via `ExtractEmbeddedTextures` for `.glb` buffer-view and base64 data-URI images), `FontAsset` (TTF glyph atlas via stb_truetype).
