...
namespace Loom {
    ...
    public:
        ...
        static void DrawQuad(const glm::mat4& transform, const std::shared_ptr<Texture2D>& texture, const glm::vec4& tint_color = glm::vec4(1.0f), float tiling_factor = 1.0f, int entity_id = -1);
        static void DrawQuad(const glm::mat4& transform, const std::shared_ptr<Texture2D>& texture, const glm::vec2 tex_coords[4], const glm::vec4& tint_color = glm::vec4(1.0f), int entity_id = -1);
